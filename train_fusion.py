...
class Trainer(object):
    def __init__(self, mode):
        ...
        self.model_fusion = LBP.BNBilinear(self.embedding_dim, self.embedding_dim, 512)
        ...
        self.lomgridtestloader = DataLoader(self.lomgridtestset, batch_size = 1, shuffle = False, num_workers = 16)
        ...
        if self.train_opts['loss'] == 'CrossEntropy':
            self.criterion = CrossEntropy(1024, n_spk).to(self.device)
    ...
    def model_average(self, avg_num = 2):
        model_state_dict = {}
    ...
    def load_finetune(self):
        ...
        if self.mode == 'av_fusion':
            if self.resume_fusion != 'None':
                self.log_time = self.resume_fusion.split('/')[1]
    ...
    def extract_test_xv_lomgrid(self):
        ...
        os.makedirs('exp/{}/test_em/test_em_lomgrid'.format(self.log_time), exist_ok = True)
        with torch.no_grad():
            for input_video, input_audio, utt in tqdm(self.lomgridtestloader):
                ...
                em = torch.cat([xv_audio,em_video], dim=1)
                # em = xv_audio
                # em = em_video
                em = em.cpu().numpy()
                test_spk_dir = os.path.join('exp/{}/test_em/test_em_lomgrid'.format(self.log_time), os.path.dirname(utt))
    ...
    def extract_test_xv_grid(self):
        ...
        # self._print_config(self.voxtestset.opts)
        os.makedirs('exp/{}/test_em/test_em_grid'.format(self.log_time), exist_ok = True)
        with torch.no_grad():
            for input_video, input_audio, utt in tqdm(self.gridtestloader):
                ...
                em = em.cpu().numpy()
                test_spk_dir = os.path.join('exp/{}/test_em/test_em_grid'.format(self.log_time), os.path.dirname(utt))
                os.makedirs(test_spk_dir, exist_ok = True)
                np.save(os.path.join(test_spk_dir, os.path.basename(utt).replace('.wav', '.npy')), em)
        self.model_audio = parallel_model_audio
        self.model_video = parallel_model_video
...
if __name__ == '__main__':
    ...
    # audio-visual dataset test
        ...
        if trainer.test_opts['eval_lomgrid']:
            trainer.extract_test_xv_lomgrid()
            if trainer.test_opts['use_cos']:              
                eer, threshold = utils.eer_cos_lomgrid_scorefusion(trainer.log_time)
                print("EER: {:.6f}%".format(eer * 100))
            if trainer.test_opts['use_plda']:              
                eer, threshold = utils.eer_plda_lomgrid(trainer.log_time)
                print("EER: {:.6f}%".format(eer * 100))
        if trainer.test_opts['eval_grid']:
            trainer.extract_test_xv_grid() 
            if trainer.test_opts['use_cos']:
                eer, threshold = utils.eer_cos_grid_scorefusion(trainer.log_time)
                print("EER: {:.6f}%".format(eer * 100))
            if trainer.test_opts['use_plda']:
                eer, threshold = utils.eer_plda_grid(trainer.log_time)
                print("EER: {:.6f}%".format(eer * 100))
